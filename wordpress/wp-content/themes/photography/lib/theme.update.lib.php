...
if(!empty($pp_envato_username) && !empty($pp_envato_api_key))
{
	...
	{
		add_action('admin_notices', 'photography_admin_notice');

		function photography_admin_notice() {
			global $current_user ;
		        $user_id = $current_user->ID;

			if ( ! get_user_meta($user_id, 'pp_ignore_notice') ) {
		        echo '<div class="updated"><p>'; 
		        printf(esc_html__(' There is update available for '.THEMENAME.' theme. Go to "Theme Setting > Auto update" tab to update the theme. | <a href="%1$s">Hide</a>'), '?pp_ignore_notice=0');
		        echo "</p></div>";
			}
		}
		
		add_action('admin_init', 'photography_nag_ignore');
		
		function photography_nag_ignore() {
			global $current_user;
		        $user_id = $current_user->ID;
